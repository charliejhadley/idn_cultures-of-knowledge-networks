...
visNetwork_select_individuals_neighboring_nodes <- reactive({
  ...
  if(nrow(visNetwork_select_individuals_edges()) == 0){
    return()
  }
...
output$selected.individual.network_no_graph <- renderUI({
  ...
  # }
  # 
  message_to_show <- wellPanel(HTML(
    paste0(
      "<p>",
